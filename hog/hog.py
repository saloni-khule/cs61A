...
def hefty_hogs(player_score, opponent_score):
    ...
    if opponent_score==0:
        ...
        while opponent_score>0:
            func_dig = opponent_score%10
            score = digit_fn(func_dig)(score)
            opponent_score//=10
    return score % 30
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    total1=0
    while num_rolls <= 10:
        total2= make_averaged(roll_dice, total_samples)(num_rolls,dice)

        if total1 < total2:
            required_rolls = num_rolls
            total1=total2
        num_rolls+=1
...
def hog_pile_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    if (hefty_hogs(score,opponent_score) + score)%10 == opponent_score%10 and opponent_score%10 !=0:
        return 0
    else:
        return hefty_hogs_strategy(score, opponent_score, threshold=threshold, num_rolls=num_rolls)
