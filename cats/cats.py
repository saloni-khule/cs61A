...
def choose(paragraphs, select, k):
    ...
    """
    # BEGIN PROBLEM 1
    list_new = [x for x in paragraphs if select(x)==True]
    if k< len(list_new):
        return list_new[k]
    else:
        return ""
...
def about(topic):
    ...
    # BEGIN PROBLEM 2
    #topic = list(topic)
    def paragraph(para_words):
        i=0
        ans=[]

        para_words=lower(para_words)
        para_words=remove_punctuation(para_words)
        para_words=para_words.split()
...
def accuracy(typed, reference):
    ...
    while i<length:
        if typed_words[i]==reference_words[i]:
            new_list.append(typed_words[i])
        i+=1


    return len(new_list)*100/len(typed_words)
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    while n<len(word_list):
        ...
        required_len.append(diff_function(typed_word,word_list[n],limit))
        minimum=min(tuple(required_len))
        required_pos = required_len.index(minimum)
        ans=word_list[required_pos]
...
def sphinx_swaps(start, goal, limit):
    ...
    if limit<0:
        ...
        ans= sphinx_swaps(start[1:],goal[1:],limit-1) +1
        return ans
        
    else:
        ans= sphinx_swaps(start[1:],goal[1:],limit) 
...
def minimum_mewtations(start, goal, limit):
    ...
    if start[0]==goal[0]:
        ...
        add = ''.join(add) # Fill in these lines
        remove = list(start)
        remove.pop(0)
        remove=''.join(remove)
        substitute = list(start)
        substitute[0]=goal[0]

        substitute=''.join(substitute)

        # BEGIN
        return min(minimum_mewtations(add,goal,limit-1), minimum_mewtations(remove,goal,limit-1),minimum_mewtations(substitute,goal,limit-1)) + 1
...
def time_per_word(words, times_per_player):
    ...
    while i<len(times_per_player):
        x=0
        jk=[]
        while x+1<len(times_per_player[i]):
        
    
            jk.append(times_per_player[i][x+1]-times_per_player[i][x])
        
            x+=1
        ans.append(jk)
        i+=1   
    times_per_player=ans
    # match = {'words': words, 'times': times_per_player}
...
def fastest_words(match):
    ...
    list_by_player = [[] for x in player_indices]

    for i in word_indices:
        minimum , min_index= time(match,0,i), 0
        for j in player_indices:
            if time(match, j,i)<minimum:
                minimum ,min_index= time(match,j,i),j
        list_by_player[min_index].append(word_at(match,i))
