...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8
            # self = ant being added
            # plact.ants = ant originally occupying place
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        time = 0
        while curr_place.is_hive != True:
            if curr_place.bees != [] and self.min_range <= time <= self.max_range:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if self.health == 0:
            for i in curr_place.bees[:]:
                Ant.reduce_health(i, (amount + self.damage))
        else:
            for i in curr_place.bees[:]:
                Ant.reduce_health(i, amount)
        # END Problem 5
    ...
    def action(self,gamestate):
        if self.chew_timer == 0:
            if self.place.bees == []:
                ...
                track = chosen_bee.health
                chosen_bee.reduce_health(track)
...
# END Problem 12
    ...
    def action(self, gamestate):
        ...
        exit_place = self.place
        exit_place = exit_place.exit

        while exit_place != None:

            if exit_place.ant:
                
                Ant.double(exit_place.ant)
                if exit_place.ant.is_container and exit_place.ant.ant_contained:
                    Ant.double(exit_place.ant.ant_contained)
            exit_place = exit_place.exit
        # END Problem 12
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        other_destination=self.place.entrance



        # Extra credit: Special handling for bee direction
        if self.slow_tracker>0 and gamestate.time%2!=0:
        ...
        if self.backward_length>0:
            self.backward_length-=1
            if self.blocked():
                self.sting(self.place.ant)
            else:

               
                self.move_to(other_destination)
            return
    ...
    def scare(self, length):
        ...
        """
        # BEGIN Problem EC
        if self.was_scared==False:
            self.backward_length+=length
